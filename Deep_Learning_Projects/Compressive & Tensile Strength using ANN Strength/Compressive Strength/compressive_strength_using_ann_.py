# -*- coding: utf-8 -*-
"""Compressive Strength using ANN .ipynb
...
# Load your data
compr_df = pd.read_excel('Compressive_Tensile_Data.xlsx', sheet_name='Compressive Strength')
...
compr_df.shape    # Outputs the number of rows and columns in the dataset.
...
missing_values = compr_df.isnull().sum()
...
new_order = [
    "Sample Type",
    "Compressive_Load_7D(KN)",
    "Compressive_Load_14D(KN)",
    "Compressive_Load_21D(KN)",
    "Compressive_Load_28D(KN)",
    "Compressive_Strength_7D(N/mm2)",
    "Compressive_Strength_14D(N/mm2)",
    "Compressive_Strength_21D(N/mm2)",
    "Compressive_Strength_28D(N/mm2)"
]

compr_df_new_ordr = compr_df.copy()

# Reorder the DataFrame
compr_df_no = compr_df_new_ordr[new_order]
...
print(compr_df_no.columns)

# Checking the distribution of Compressive Load at 7 days
sns.distplot(compr_df_no['Compressive_Load_7D(KN)'],color='green')

# List of days
days = ["7D", "14D", "21D", "28D"]

# Loop over each day and plot both Compressive Load and Strength
for day in days:
    # Plot Compressive Load
    load_col = f"Compressive_Load_{day}(KN)"
    if load_col in compr_df_no.columns:
        sns.histplot(compr_df_no[load_col], kde=True, color='green')
        plt.title(f"Distribution of {day} Compressive Load")
        plt.xlabel("Compressive Load (KN)")
        plt.ylabel("Frequency")
        plt.show()

    # Plot Compressive Strength
    strength_col = f"Compressive_Strength_{day}(N/mm2)"
    if strength_col in compr_df_no.columns:
        sns.histplot(compr_df_no[strength_col], kde=True, color='red')
        plt.title(f"Distribution of {day} Compressive Strength")
        plt.xlabel("Compressive Strength (N/mm2)")
        plt.ylabel("Frequency")
        plt.show()

correlation = compr_df_no['Compressive_Load_7D(KN)'].corr(compr_df_no['Compressive_Strength_7D(N/mm2)'])
print("Correlation between 7D Load and Strength:", correlation)

correlation = compr_df_no['Compressive_Load_14D(KN)'].corr(compr_df_no['Compressive_Strength_14D(N/mm2)'])
print("Correlation between 14D Load and Strength:", correlation)

correlation = compr_df_no['Compressive_Load_21D(KN)'].corr(compr_df_no['Compressive_Strength_21D(N/mm2)'])
print("Correlation between 21D Load and Strength:", correlation)

correlation = compr_df_no['Compressive_Load_28D(KN)'].corr(compr_df_no['Compressive_Strength_28D(N/mm2)'])
print("Correlation between 28D Load and Strength:", correlation)

# Copy
compr_df_new_ordr = compr_df_no.copy()

# Drop the Compressive Load
cols_to_drop = [col for col in compr_df_new_ordr.columns if col.startswith("Compressive_Load_")]
compr_df_new_ordr = compr_df_new_ordr.drop(columns=cols_to_drop)


# Display the first few rows
print("First 5 rows of the dataset:")
compr_df_new_ordr.head()
...
# Define the actual compressive strength feature names
continuous_feature = [
    'Compressive_Strength_7D(N/mm2)',
    'Compressive_Strength_14D(N/mm2)',
    'Compressive_Strength_21D(N/mm2)',
...
for feature in continuous_feature:
    sns.histplot(compr_df_new_ordr[feature], bins=25, kde=True, color='skyblue')
...
# Statistical description of the dataset
print("\nStatistical Description of the Dataset:")
compr_df_new_ordr.describe()

"""### **Correlation**"""

# Copy
compr_df_nw_odr = compr_df_new_ordr.copy()

# Drop or Exclude Non-Numeric Columns
numeric_data = compr_df_nw_odr.select_dtypes(include=['float64', 'int64'])
...
target_col = 'Compressive_Strength_28D(N/mm2)'
...
for col in strength_columns:
    ...
    sns.violinplot(data=compr_df_new_ordr, x='Sample Type', y=col, palette='Set2')
...
for col in strength_columns:
    plt.figure(figsize=(10, 6))
    sns.boxplot(data=compr_df_new_ordr, x='Sample Type', y=col, palette='Set3')
    plt.title(f"Effect of Sample Type on {col}")
    plt.xlabel("Sample Type")
    plt.ylabel("Compressive Strength (N/mm²)")
    plt.xticks(rotation=45)
...
X = df.drop(['Compressive_Strength_28D(N/mm2)', "Compressive_Load_28D(KN)"], axis=1)
y = df['Compressive_Strength_28D(N/mm2)']
...
plt.plot([y_test.min(), y_test.max()],
         ...
         'r--')
plt.xlabel("True 28D Strength (N/mm²)")
plt.ylabel("Predicted 28D Strength (N/mm²)")
